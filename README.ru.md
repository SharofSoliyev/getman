...
**[Скачать последний релиз](https://github.com/SharofSoliyev/getman/releases/latest)** — самодостаточные исполняемые файлы, без установщика и без отдельной среды выполнения.
При первом запуске Windows покажет предупреждение — [почему и что делать](#windows-protected-your-pc).
...
Написан на **WPF и .NET 9**, оформлен с помощью **WPF-UI** (Fluent 2).
...
dotnet publish src/GetMan -c Release -r win-x64 --self-contained true \
  ...
  -p:IncludeNativeLibrariesForSelfExtract=true -o dist        # один файл GetMan.exe
dotnet publish src/GetMan.Cli -c Release -r win-x64 --self-contained true \
  -p:PublishSingleFile=true -p:EnableCompressionInSingleFile=true \
  -p:IncludeNativeLibrariesForSelfExtract=true -o dist-cli    # консольный getman.exe
```

Рабочее пространство хранится в `%APPDATA%\GetMan\workspace.json` (с резервной копией), отдельно у
каждого пользователя Windows. Никуда ничего не отправляется, а пароли, токены и ключи шифруются на
диске через Windows DPAPI — что это защищает, а что нет, описано в [SECURITY.md](SECURITY.md).

## Платформы

| | Windows | Linux | macOS |
|---|:---:|:---:|:---:|
| **GetMan** — приложение | ✅ x64 | ✕ | ✕ |
| **getman** — [консольный раннер](#запуск-коллекций-из-командной-строки) | ✅ x64 | ✅ x64, arm64 | ✅ x64, arm64 |

Приложение написано на WPF, а WPF — технология Windows: сборки для Linux и macOS не существует и не
появится без переписывания интерфейса на кросс-платформенном тулките. Это полноценная работа, а не
переключатель, поэтому честный ответ сегодня — только Windows.

**Консольный раннер кросс-платформенный**, и это как раз там, где важнее всего: он нужен для запуска
коллекций в CI, а CI в основном работает на Linux. Он разделяет с приложением только слои модели и
сервисов — никакого WPF, — и CI это доказывает, прогоняя на Ubuntu весь набор сервисных тестов и сам
раннер при каждом коммите, а не просто компилируя их.

## Установка на Windows

Скачайте `GetMan-<версия>-win-x64.exe` со страницы
[Releases](https://github.com/SharofSoliyev/getman/releases/latest) и запустите. Установщика нет и
распаковывать нечего: это один самодостаточный исполняемый файл, а рабочее пространство он хранит
в `%APPDATA%\GetMan`.

### «Windows protected your PC»

При первом запуске появится синее окно SmartScreen, в котором видна только кнопка **Не выполнять**.
Нажмите **Подробнее**, затем **Выполнить в любом случае**. Windows запомнит ответ, и больше окно не
появится.

Это не предупреждение об угрозе и оно ничего не говорит о том, что файл делает. SmartScreen задаёт
два вопроса — кто подписал файл и сколько людей его уже запускали — и неподписанный файл небольшого
проекта не может ответить ни на один. То же самое окно получает любая неподписанная загрузка любого
небольшого проекта.

Окно перестанет появляться у всех только тогда, когда двоичные файлы будут подписаны сертификатом
подписи кода, а это не настройка сборки, а постоянные расходы: Azure Trusted Signing стоит около
$10 в месяц, обычный OV-сертификат — несколько сотен долларов в год плюс аппаратный токен. Это и
есть честная причина, почему подписи пока нет. Шаг подписи уже присутствует
в [рабочем процессе релиза](.github/workflows/release.yml): он пропускается, пока у репозитория нет
секретов учётной записи, и включается сам, как только они появятся, — переписывать релиз не
придётся.

А пока — проверяйте загрузку, а не доверяйте окну. В каждом релизе есть `SHA256SUMS.txt`:

```powershell
Get-FileHash .\GetMan-1.1.0-win-x64.exe -Algorithm SHA256
```

Если хеш совпадает со строкой для этого файла в `SHA256SUMS.txt`, значит исполняемый файл побайтово
совпадает с тем, что собрал GitHub Actions из этого репозитория — сборка, которую можно прочитать,
на раннере, лог которого виден. Это более надёжная гарантия, чем когда-либо дало бы окно
SmartScreen.
...
### Почему GetMan не читает базу самого Postman

GetMan обнаруживает установленный Postman и сообщает его версию, но его хранилище не читает —
начиная с Postman 10 там нечего читать:

- **Сохранённых запросов на диске нет.** Они живут в вашем аккаунте Postman. Именно их забирает
  путь через аккаунт выше, а `GET /collections` охватывает все воркспейсы, доступные ключу, так что
  ничего сохранённого не теряется.
- **Локальная Chromium IndexedDB хранит состояние приложения**, а не тела запросов: какая панель
  открыта, какая вкладка активна, и кэш *значений* окружений. На машине с восемью коллекциями и
  примерно двумя сотнями запросов все локальные хранилища вместе содержали двенадцать не-postman
  URL, и все они оказались значениями переменных окружения.
- **Несохранённые правки в открытой вкладке живут в памяти** и на диск не попадают вовсе.

Поэтому запрос, открытый в Postman, но не сохранённый, не сможет вытащить ничто, читающее диск.
Сохраните вкладку в Postman — и импорт через аккаунт заберёт её мгновением позже, из того же облака.
`Settings → Data → Export Data` — второй полный путь: один дамп со всем, что есть у Postman, включая
локальные коллекции и Scratch Pad, в формате, который GetMan уже читает.

## Что GetMan умеет импортировать

**Импорт** — кнопка `Импорт`, `Ctrl+O` или `Вставка / cURL` для произвольного текста. Формат
определяется по содержимому, поэтому одна и та же кнопка принимает всё это:

| Формат | Поддержка |
|---|---|
| Коллекция Postman v2.1 | да |
| Коллекция Postman v2.0 | да |
| Коллекция Postman v1 (`requests` + `folders`) | да |
| Экспорт окружения / глобальных переменных Postman | да |
| Дамп «Export data» из Postman (`collections` + `environments` + `globals`) | да |
| **OpenAPI 3.0 / 3.1**, JSON или YAML | да |
| **Swagger 2.0**, JSON или YAML | да |
| Команда cURL (кавычки bash или cmd) | да |

### OpenAPI и Swagger

Укажите GetMan на `swagger.json` или `openapi.yaml` — и получите рабочую коллекцию, а не список URL:

- **Одна папка на тег.** Операция без тега попадает в папку по первому сегменту пути.
- **`servers` становится `{{baseUrl}}`** — и переменной коллекции, *и* окружением, так что
  переключение между staging и production превращается в выпадающий список. Шаблонный сервер вроде
  `https://{region}.api.example.com` делает `region` отдельной переменной со значением по умолчанию
  из описания.
- **Параметры становятся строками таблиц.** Обязательные query-параметры попадают в URL,
  необязательные — в отключённые строки, которые вы включаете при необходимости. `path`-параметры
  превращаются в сегменты `:name`, `header`-параметры — в заголовки, и все сохраняют описание.
- **Тело запроса генерируется из схемы** — `$ref` разыменовывается, `allOf` сливается,
  `oneOf`/`anyOf` берут первую ветку, `example`, `default` и `enum` предпочитаются заглушкам,
  а форматы превращаются в правдоподобные значения (`date` → `2026-01-31`, `uuid` → `{{$guid}}`,
  `email` → `user@example.com`). Схема, ссылающаяся на саму себя, завершается, а не зацикливается.
  `multipart/form-data` становится form-data, где поля с `format: binary` — файловые строки;
  `application/x-www-form-urlencoded` становится urlencoded-телом.
- **Схемы безопасности становятся авторизацией.** `http bearer`, `http basic`, `apiKey`
  (в заголовке или в query) и `oauth2` ложатся на авторизацию GetMan, а учётные данные остаются
  пустыми переменными коллекции: в описании секрета нет, и GetMan его не выдумывает. Требование,
  объявленное на отдельной операции, переопределяет умолчание коллекции.

Всё, что перенести не удалось, сообщается после импорта, а не выбрасывается молча: второй сервер,
тип тела, который GetMan не строит, схема безопасности без эквивалента.

**Не импортируется:** WSDL, HAR, Insomnia. Это отдельные форматы.
...
Экспортированные файлы чисто импортируются обратно и в Postman, и в GetMan — включая коллекцию,
которая началась как описание OpenAPI.
...
а не превращаются в пустоту, — так видно, чего не хватает. Если набрать `{{` в любом поле — URL,
заголовок, токен авторизации — выпадет список того, что доступно прямо сейчас, со значениями и
областью видимости: выбираете, а не вспоминаете. Ctrl+Space открывает тот же список где угодно.
...
**Секреты шифруются на диске** — пароли, токены и ключи запечатываются через Windows DPAPI до
записи в файл, а экспорт остаётся открытым, чтобы он по-прежнему открывался в Postman.
...
Интерфейс следует дизайн-системе **инструмента разработчика / IDE** поверх Fluent 2
(WPF-UI, MIT), с подложкой Mica на Windows 11.
...
GetMan.exe --audit out-dir                               # каждое окно в обеих темах — чтобы заметить
                                                         # проблему с контрастом раньше пользователя
...
[YamlDotNet](https://github.com/aaubry/YamlDotNet) (MIT) и
